...
const MembershipRules = ({ onShowForm }) => (
  <Container className='border-[#fee57e] border-8'>
    <motion.h2
    ...
    >
      <span className='text-4xl font-bold text-[#280101] underline-travel md:text-6xl'>Rules for being a member of MAATI-16</span>
    </motion.h2>
    <ul>
    ...
    <SubmitButton onClick={onShowForm} className='bg-amber-500 hover:bg-amber-600'>View Form</SubmitButton>
  </Container>
);
...
const Form = () => {
  ...
  return (
    <FormContainer className='border-[#fee57e] border-8'>
      <motion.h2
        className="pt-12 pb-5 mb-4 text-4xl font-bold text-center"
        initial={{ y: -50, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
...
const App = () => {
  ...
  return (
    <div>
      {!showForm ? (
        <MembershipRules onShowForm={() => setShowForm(true)} />
      ) : (
        <Form />
      )}
    </div>
